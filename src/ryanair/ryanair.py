...
class Ryanair:
    ...
    def _retryable_query(self, url, params=None):
        self._num_queries += 1
        trip_type = "return" if "roundTripFares" in url else "one-way"
        origin = params.get('departureAirportIataCode', 'N/A')
        date = params.get('outboundDepartureDateFrom', 'N/A')
        dest = params.get('arrivalCountryCode', 'any')
        logger.info(f"API#{self._num_queries}: {trip_type} flight {origin} ({date})-> {dest}")
    ...
    def _parse_cheapest_flight(self, flight):
        ...
        try:
            return Flight(
                ...
                arrivalTime=datetime.fromisoformat(flight["arrivalDate"]),
                flightNumber=f"{flight['flightNumber'][:2]} {flight['flightNumber'][2:]}",
                price=flight["price"]["value"],
                currency=currency,
            )
        except Exception as e:
            logger.error(f"Error parsing flight data: {str(e)}", exc_info=True)
            raise RyanairException(f"Failed to parse flight data: {str(e)}")
    ...
    def _parse_flight(self, flight_data: dict) -> Flight:
        departure_time = datetime.fromisoformat(flight_data["departureDate"])
        arrival_time = datetime.fromisoformat(flight_data["arrivalDate"])
        
        return Flight(
            departureTime=departure_time,
            arrivalTime=arrival_time,
            flightNumber=flight_data["flightNumber"],
            price=flight_data["price"]["value"],
            currency=flight_data["price"]["currencyCode"],
            origin=flight_data["departureAirport"]["iataCode"],
            originFull=f"{flight_data['departureAirport']['city']['name']}, {flight_data['departureAirport']['countryName']}",
            destination=flight_data["arrivalAirport"]["iataCode"],
            destinationFull=f"{flight_data['arrivalAirport']['city']['name']}, {flight_data['arrivalAirport']['countryName']}"
        )
